...
void InterpLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  interp_param_ = this->layer_param_.interp_param();
  int num_specs = 0;
  num_specs += interp_param_.has_zoom_factor();
  num_specs += interp_param_.has_shrink_factor();
  num_specs += interp_param_.has_height() && interp_param_.has_width();
  num_specs += interp_param_.has_use_bottom();
  CHECK_EQ(num_specs, 1) << "Output dimension specified either by "
			 << "zoom factor or shrink factor or explicitly or parametrically "
			 << "using the bottom blob";
  pad_beg_ = interp_param_.pad_beg();
  pad_end_ = interp_param_.pad_end();
...
void InterpLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if (interp_param_.has_zoom_factor()) {
    const int zoom_factor = interp_param_.zoom_factor();
  ...
  else if (interp_param_.has_shrink_factor()) {
    const int shrink_factor = interp_param_.shrink_factor();
  ...
  else if (interp_param_.has_height() && interp_param_.has_width()) {
    height_out_  = interp_param_.height();
    width_out_  = interp_param_.width();
  } else if(interp_param_.has_use_bottom() && interp_param_.use_bottom()) {
    // If you use the parameters from the bottom
    // Then right now you are not sure what would be the output values
    // So just make it equal to the effective input values
    height_out_ = height_in_eff_;
    width_out_ = width_in_eff_;
  }
  else {
...
void InterpLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if(interp_param_.has_use_bottom() && interp_param_.use_bottom()) {
    num_ = bottom[0]->num();
    channels_ = bottom[0]->channels();
    height_in_ = bottom[0]->height();
    width_in_ = bottom[0]->width();
    height_in_eff_ = height_in_ + pad_beg_ + pad_end_;
    width_in_eff_ = width_in_ + pad_beg_ + pad_end_;
    const Dtype* dim_data = bottom[1]->cpu_data();
    height_out_ = static_cast<int>(dim_data[0]);
    width_out_ = static_cast<int>(dim_data[1]);
...
void InterpLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if(interp_param_.has_use_bottom() && interp_param_.use_bottom()) {
    ...
    height_in_eff_ = height_in_ + pad_beg_ + pad_end_;
    width_in_eff_ = width_in_ + pad_beg_ + pad_end_;
    const Dtype* dim_data = bottom[1]->gpu_data();
    height_out_ = static_cast<int>(dim_data[0]);
    width_out_ = static_cast<int>(dim_data[1]);
